...
document.addEventListener("DOMContentLoaded", function () {
  ...
  if (floatingMenu) {
    ...
    main.style.paddingTop = `${floatingMenuHeight}px`;
  }
});
...
function initSpoilerBody(spoilersBlock, hideSpollerBody = true) {
...
function hideSpollerBody(spoilersBlock) {
...
const menuList = document.querySelectorAll(".hide");
...
function popupOpen(currentPopup) {
  if (currentPopup && unlock) {
    ...
    if (currentPopup.classList.contains("side-menu")) {
      if (menuList.length > 0) {
        for (let index = 0; index < menuList.length; index++) {
          menuList[index].style.visibility = "hidden";
          menuList[index].style.opacity = "0";
        }
      }
    }
...
function popupClose(popupActive, doUnlock = true) {
  if (unlock) {
    ...
    if (popupActive.classList.contains("side-menu")) {
      if (menuList.length > 0) {
        for (let index = 0; index < menuList.length; index++) {
          menuList[index].style.visibility = "visible";
          menuList[index].style.opacity = "1";
        }
      }
    }
    if (doUnlock) {
...
const subscribePlanAll = document.querySelectorAll(".subscribe__plan");

if (subscribePlanAll.length > 0) {
  for (let index = 0; index < subscribePlanAll.length; index++) {
    subscribePlanAll[index].addEventListener("click", function () {
      for (let i = 0; i < subscribePlanAll.length; i++) {
        subscribePlanAll[i].classList.remove("active");
        subscribePlanAll[i].classList.add("hidden");
      }
      // Добавляем класс "active" только к текущему элементу

      const allButtons = document.querySelectorAll(".subscribe__plan-btn");
      for (let button of allButtons) {
        button.classList.remove("active");
      }

      // Добавляем класс "active" к ссылке внутри текущего активного элемента
      const buttonInside = this.querySelector(".subscribe__plan-btn");
      if (buttonInside) {
        buttonInside.classList.add("active");
      }
      ...
      const inputInside = this.querySelector("input[type='checkbox']");
      if (inputInside) {
